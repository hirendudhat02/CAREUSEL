...
export default class CometChatMessageComposer extends React.PureComponent {
  ...
  checkRestrictions = async () => {
    ...
    let isTypingIndicatorsEnabled = true
   ...
   */
  sendMultipleMediaMessage = (messageInput, messageType) => {
    try {
      ...
      mediaMessage.setData(mediaData);
  ...
  sendTextMessage = () => {
    try {
      ...
      textMessage._composedAt = new Date();
  ...
  sendSticker = stickerMessage => {
    ...
    customMessage.setConversationId(conversationId);
    customMessage._composedAt = new Date();
  ...
  render() {
    ...
    let sendBtn = (
      <TouchableOpacity
        disabled={!this.state.messageInput.length ? true : false}
        ...
        <View
          ...
          <Text
            ...
            {this.props.type === CometChat.RECEIVER_TYPE.GROUP
              ? 'You are no longer a part of this group or \n this group doesn’t exist anymore.'
              : 'You are no longer connected with \n this User.'}
          </Text>
        </View>
      );
    }

    var smartReplyPreview = null;
        ...
        {insets => (
          <View
            style={{
              ...
              backgroundColor: '#fff',
              paddingBottom:
                Platform.OS === 'ios' ? insets.bottom : 10 * heightRatio,
            }}>
            {blockedPreview}
            {editPreview}
            {createPoll}
            ...
            <ComposerActions
              ...
              sendMultipleMediaMessage={this.sendMultipleMediaMessage}
            />
            {!disabled && (<View style={style.mainContainer}>
              ...
              <TouchableOpacity
                ...
                <Entypo size={18} name="attachment" color="#fff" />
              </TouchableOpacity>
              {sendBtn}
            </View>)}
