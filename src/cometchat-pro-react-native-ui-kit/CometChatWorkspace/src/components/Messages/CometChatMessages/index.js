...
class CometChatMessages extends React.Component {
  ...
  componentDidMount() {
    ...
    var check = false;
    if (this.props.AlluserList !== undefined) {
      if (this.props.AlluserList.length !== 0) {
        this.props.AlluserList.forEach(element => {
          if (element.userId === receiverId) {
            check = true;
          }
        });
      }
  ...
  membersAdded = members => {
    ...
    members.forEach(eachMember => {
      const message = `${eachMember.name} added to ${this.loggedInUser.name}`;
      const sentAt = (new Date() / 1000) | 0;
      const messageObj = {
        category: 'action',
        message,
  ...
  appendMessage = (newMessages = []) => {
    ...
    var newMessagesData = messages.concat(newMessages);

    this.setState({ messageList: newMessagesData, scrollToBottom: true });
  };
          ...
          {insets => (
            <KeyboardAvoidingView
              ...
              keyboardVerticalOffset={
                Platform.OS === 'ios' ? verticalScale(0) : verticalScale(80)
              }
              style={{ flexGrow: 1 }}>
              <View
                style={[
