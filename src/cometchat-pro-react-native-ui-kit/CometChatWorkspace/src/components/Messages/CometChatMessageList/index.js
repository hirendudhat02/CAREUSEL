...
class CometChatMessageList extends React.PureComponent {
        ...
        this.MessageListManager.fetchPreviousMessages()
          .then(messageList => {
            if (messageList.length === 0) {
              this.decoratorMessage = 'You have not exchanged any messages yet';
            }
  ...
  getSenderMessageComponent = (message, key) => {
    ...
    } else if (Object.prototype.hasOwnProperty.call(message, 'editedAt')) {
      component = (
  ...
  renderItem = ({ item, index }) => {
    ...
    if (cDate !== messageSentDate && messages[0].sentAt) {
      var dayFormat = moment(
        new Date(messages[index].sentAt * 1000).toJSON(),
      ).format('dddd');
      ...
      var msDiff =
        new Date().getTime() -
        new Date(new Date(messages[index].sentAt * 1000).toJSON()).getTime();
      ...
      if (daysTill30June2035 === 0) {
        ...
        dateValue = 'Yesterday';
      } else if (daysTill30June2035 >= 2 && daysTill30June2035 <= 7) {
        dateValue = dayFormat;
      } else {
